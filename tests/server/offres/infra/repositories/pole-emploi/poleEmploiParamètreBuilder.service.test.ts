import { anOffreEchantillonAvecLocalisationEtMotCléFiltre } from '@tests/fixtures/domain/offre.fixture';
import {
...
describe('poleEmploiParamètreBuilder.service', () => {
  ...
  describe('buildCommonParamètresRecherche', () => {
    it('retourne les paramètres communs au recherche de l’api pole emploi', async () => {
      ...
      const result = await new PoleEmploiParamètreBuilderService(apiPoleEmploiRéférentielRepositoryMock).buildCommonParamètresRecherche(anOffreEchantillonAvecLocalisationEtMotCléFiltre());
    ...
    it('quand la page dépasse le range max renvoie undefined', async () => {
      const result = await new PoleEmploiParamètreBuilderService(apiPoleEmploiRéférentielRepositoryMock).buildCommonParamètresRecherche(anOffreEchantillonAvecLocalisationEtMotCléFiltre({ page: 1001 }));
