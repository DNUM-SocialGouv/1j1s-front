	...
	describe('lorsque la recherche est valide', () => {
		it('retourne la liste des établissements d‘accompagnement', async () => {
			nock('https://api-lannuaire.service-public.fr/api/explore/v2.1')
				.get('/catalog/datasets/api-lannuaire-administration/records?where=suggest(adresse,%22code_postal%2075011%22)and%20pivot%20LIKE%20%22cij%22&limit=100&select=adresse,telephone,adresse_courriel,nom,id,pivot,plage_ouverture')
			...
			await testApiHandler<EtablissementAccompagnement[] | ErrorHttpResponse>({
				...
				url: '/etablissements-accompagnement?codePostal=75011&typeAccompagnement=cij',
			});
		});
	});
