...
describe('StrapiMentionObligatoireRepository', () => {
	it('appelle le service CMS avec les bon paramètres', () => {
		...
		vi.spyOn(cmsService, 'getSingleType').mockResolvedValueOnce(createSuccess(cmsServiceResponse));
	...
	it('retourne la mention obligatoire', async () => {
		// GIVEN
		const cmsService = aStrapiService();
		vi.spyOn(cmsService, 'getSingleType').mockResolvedValueOnce(createSuccess(aStrapiMentionObligatoire({
