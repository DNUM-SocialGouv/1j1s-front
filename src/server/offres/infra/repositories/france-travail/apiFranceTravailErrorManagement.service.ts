...
export const errorFromApiFranceTravailSearch = [
...
export const errorFromApiFranceTravailGet = 'Le format de l\'id de l\'offre recherchée est incorrect.';

export class ApiFranceTravailOffreErrorManagementServiceSearch extends DefaultErrorManagementService {
	...
	protected handleHttpErrorFailure(error: HttpError, logInformation: LogInformation) {
		if (error.response?.status === 400 && errorFromApiFranceTravailSearch.includes(error.response.data?.message)) {
...
export class ApiFranceTravailOffreErrorManagementServiceGet extends DefaultErrorManagementService implements ErrorManagementWithErrorCheckingService {
	handleFailureError(error: unknown, logInformation: LogInformation) {
	...
	protected handleHttpErrorFailure(error: HttpError, logInformation: LogInformation) {
		if (error.response?.status === 400 && error.response.data?.message === errorFromApiFranceTravailGet) {
			const warnToLog = super.buildHttpErrorToLog(logInformation, error);
			this.logError(warnToLog, Severity.WARNING);
		} else {
			this.logHttpError(error, logInformation);
