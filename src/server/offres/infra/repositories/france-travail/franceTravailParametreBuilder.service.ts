...
export class FranceTravailParametreBuilderService {

	constructor(
    private apiFranceTravailReferentielRepository: ApiFranceTravailReferentielRepository,
	) {}
		...
		if (offreEmploiFiltre.localisation) {
			...
			switch (typeLocalisation) {
				...
				case TypeLocalisation.COMMUNE: {
					const codeInseeInRéférentiel = await this.apiFranceTravailReferentielRepository.findCodeInseeInRéférentielCommune(offreEmploiFiltre.localisation.code);
