...
import {
	ListerNomMétierFicheMétierUseCase,
} from '~/server/cms/useCases/listerNomMétierFicheMétier.useCase';
...
export interface CmsDependencies {
  ...
  duréeDeValiditéEnSecondes: () => number
	listerNomMétierFicheMétierUseCase: ListerNomMétierFicheMétierUseCase
...
export function cmsDependenciesContainer(cmsRepository: StrapiCmsRepository, configurationService: ConfigurationService): CmsDependencies {
	...
	return {
		...
		listerNomMétierFicheMétierUseCase: new ListerNomMétierFicheMétierUseCase(cmsRepository),
