...
import { aStrapiIndexCmsRepository } from '~/server/cms/infra/repositories/strapiIndexCms.repository.fixture';
import {
	createFailure, createSuccess,	Failure,
	Success } from '~/server/errors/either';
import { ErreurMétier } from '~/server/errors/erreurMétier.types';
import { HttpClientService } from '~/server/services/http/httpClientService';
import {
	anAxiosError,
	anAxiosResponse,
	anHttpClientService, anHttpClientServiceWithAuthentification,
	...
	describe('saveOffreDeStage', () => {
		describe('Si un stage est fourni', () => {
			it('il est enregistré dans le cms', async () => {
				...
				expect(strapiIndexCmsRepository.save).toHaveBeenCalledWith('stages', anOffreDeStageDepot());
			});
		});
	});
});
