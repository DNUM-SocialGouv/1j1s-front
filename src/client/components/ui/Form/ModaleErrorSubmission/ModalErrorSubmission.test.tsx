...
describe('<ModalErrorSubmission/>', () => {
	it('affiche la modale quand la propriété d‘ouverture est true', () => {
		render(<ModalErrorSubmission isOpen={true} onClose={vi.fn()} onBackToForm={vi.fn()} />);
	...
	it('n‘affiche pas la modale quand la propriété d‘ouverture est à false', () => {
		render(<ModalErrorSubmission isOpen={false} onClose={vi.fn()} onBackToForm={vi.fn()} />);
	...
	it('affiche le bouton de retour au formulaire et appelle onBackToForm lorsque le bouton est cliqué', async () => {
		...
		render(<ModalErrorSubmission isOpen={true} onClose={vi.fn()} onBackToForm={onBackToForm} />);
	...
	it('appelle la propriété onClose lorsque la modale d‘erreur est fermée', async () => {
		const user = userEvent.setup();
		const onClose = vi.fn();
		render(<ModalErrorSubmission isOpen={true} onClose={onClose} onBackToForm={vi.fn()} />);
	...
	it('Lorsqu‘une description est présente, elle s‘affiche',  () => {
		const onClose = vi.fn();
		render(<ModalErrorSubmission isOpen={true} onClose={onClose} description={'je suis la description'} onBackToForm={vi.fn()} />);
